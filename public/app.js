const App = {
        ...
        try {
            ...
            const llenar = (id, data) => {
                ...
                if(el) {
                    ...
                    if(id==='filtro-familia') el.innerHTML = '<option value="TODAS">Todas las Familias</option>';
    ...
    // =============================================================
    // NUEVA FUNCIONALIDAD: VISTA DE LECTURA (MODAL SEGURO)
    // =============================================================
    verFicha: (id) => {
        ...
        // Guardar ID por si quiere editar
        document.getElementById('ver-id-oculto').value = t.id;

        // Rellenar Galería Visual
        ...
        if (t.archivos && Array.isArray(t.archivos) && t.archivos.length > 0) {
            t.archivos.forEach(arch => {
                ...
                galeria.innerHTML += `
                    <a href="${arch.url}" target="_blank" class="thumb-archivo">
                        ${preview}
    ...
    editarDesdeFicha: () => {
        ...
        App.editar(id); // Llama a la función de edición de siempre
    },
    // =============================================================

    // --- RENDER TABLA (MODIFICADO PARA USAR verFicha) ---
    renderTabla: () => {
        ...
        tbody.innerHTML = filtrados.map(t => {
            ...
            let accs = `
                <button class="btn-icono" onclick="App.editar(${t.id})" title="Editar (Cuidado)">✏️</button>
            ...
            // CAMBIO: onclick en TR llama a verFicha, NO a editar
            return `
            <tr style="${t.estado==='DESCATALOGADO'?'opacity:0.6':''}" onclick="App.verFicha(${t.id})" style="cursor:pointer;">
                ...
                <td onclick="event.stopPropagation()">${accs}</td>
            </tr>`;
        }).join('');
    },

    // --- RESTO DE FUNCIONES (IGUAL QUE V14) ---
    crearFamilia: async () => {
    ...
    buscarMovil: (txt) => { const d = document.getElementById('resultados-movil'); d.innerHTML = ""; if(txt.length<2) return; const h = App.datos.filter(t => (t.nombre+t.id_troquel+t.ubicacion).toLowerCase().includes(txt.toLowerCase())); d.innerHTML = h.slice(0,10).map(t => `<div class="card-movil" onclick="App.verFicha(${t.id})"><div style="font-weight:900;">${t.id_troquel}</div><div>${t.nombre}</div><button class="btn-secundario">Ver</button></div>`).join(''); },
